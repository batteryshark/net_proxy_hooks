...
#include "socks5.h"
	...
	bool SOCKS5__Greeting(SOCKET s, int use_async) {
		...
		if (bytes_recv == SOCKET_ERROR) { 
			...
			//DbgUtils__dbg_printf("SOCKS5 Greeting Recv Failed: %04X!",WSAGetLastError());  return false; 
		}

		if (greeting_res[0] != 0x05 || greeting_res[1] == 0xFF) {
			DbgUtils__dbg_printf("SOCKS5 Greeting Failed!");
			return false;
		}

		if (use_async) {
			Net__SocketSetNonBlocking(s);
		}
	...
	// -- Version ID (SOCKS5 so... yeah - 5)
		...
		if (send(s, (const char*)connect_req, 4 + addr_length, 0) != (4 + addr_length)) {
			...
			use_async = 1;
			if (send(s, (const char*)connect_req, 4 + addr_length, 0) != (4 + addr_length)) {
				DbgUtils__dbg_printf("Send Error!!!");
				return false;
			}
		}
		...
		if (bytes_recv == SOCKET_ERROR) {
			...
			if (bytes_recv == SOCKET_ERROR) {
				DbgUtils__dbg_printf("RECV ERROR!!!");
				return false;
			}
		}
		
		if (connect_res[0] != 0x05 || connect_res[1] != 0x00) {
			DbgUtils__dbg_printf("SOCKS5 Connect Failed!");
			return false;
		}
	...
	bool SOCKS5__Test(const struct sockaddr* dest, const struct sockaddr* proxy) {
		...
		if (!Net__ConnectTCP(proxy, &s)) { closesocket(s); return false; }
		if (!SOCKS5__Greeting(s, 0)) { closesocket(s); return false; }
		if (!SOCKS5__ConnectRequest(s, dest, 0)) { closesocket(s); return false; }
		closesocket(s);
		return true;
	}
